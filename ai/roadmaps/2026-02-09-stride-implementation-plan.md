...
**Goal:** Set up the project infrastructure and verify all integrations work.
...
**Deliverable:** Empty Next.js app that connects to Supabase, OpenAI, and Google Calendar. No UI yet, just API routes that prove integrations work.
...
**Deliverable:** User can add tasks (text only), click "Build my day", and see a schedule with tasks placed in free time slots. No photos, no goals, no dynamic updates yet.

---

## Phase 2: UI & PWA (Week 3)

**Goal:** Make the app usable on phones and visually aligned with the template.

### 2.1 Timeline/Calendar View
- Choose and integrate timeline library (e.g., react-calendar-timeline)
- Display today's calendar: busy blocks (from Google Calendar) + scheduled tasks (from AI)
- Visual distinction between calendar events and scheduled tasks
- Show overflow list (tasks that didn't fit)

### 2.2 Responsive Design
- Mobile-first layout (task input, timeline view)
- Use oatmeal-olive-instrument components and patterns
- Ensure usability on phones (touch targets, scrolling, readability)
...
**Deliverable:** Installable PWA that works on phones. Users can add tasks, build their day, and see a timeline with calendar events and scheduled tasks. Notifications for task start times.
...
**Deliverable:** Polished MVP ready for external beta. Core flow (task input → build day → timeline) is smooth and reliable.
...
1. Phase 0 (Foundation) → Phase 1 (Core Data Flow) → Phase 2 (UI & PWA)
...
- Domain name + hosting (Vercel/Netlify; can be done in Phase 2 for PWA)
...
2. **Use phase plan/roadmap docs** in `ai/roadmaps/`: each phase (0–7) has a `2026-02-09-phase-N-*-plan.md` and `-roadmap.md` pair
3. **Set up project** (Phase 0) and verify integrations
4. **Start Phase 1** (Core Data Flow) and ship the happy path ASAP
